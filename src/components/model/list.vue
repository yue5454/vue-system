        ...
        <el-table v-loading='loading' size='small' @header-click='getSortList' :data='tableList' style='width:100%'>
          ...
          <el-table-column label='Model'>
            <template slot-scope='scope'>
              <router-link :to="{path: `model_detail/${scope.row.id}`}">
                <a type='text' size='small'>{{scope.row.model}}</a>
              </router-link>
            </template>
          </el-table-column>
...
<script type="text/javascript">
  import { getModelList,deleteModel } from '@/api/api.js'
  ...
  export default {
    ...
    methods: {
      ...
      create () {
        thisVue.$router.push('/model_create');
      },
      getList () {
        thisVue.loading = true;
        getModelList().then(json => {
          thisVue.tableList = json.data.models;
      ...
      delete (id) {
        thisVue.loading = true;
        deleteModel({model_id: id}).then(json => {
