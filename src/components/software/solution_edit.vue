      ...
      <el-form ref='form' :model='form' label-width='80px' :rules='rules'>
        ...
        <div v-if='active==1'>
          <el-form-item label-width="130px" label='Solution Name' prop='solution'>
            <el-input disabled size='small' v-model='form.solution'></el-input>
          </el-form-item>
          <el-form-item label-width="130px" label='Version' prop='version'>
            <el-col :span='23'><el-input disabled size='small' v-model='form.version'></el-input></el-col>
          ...
          <el-form-item label-width="130px" label='Model Name' prop='model_id'>
            <el-select disabled size='small' v-model="form.model_id" placeholder='please select model'>
        ...
        <div v-if="active == 3">
          <el-form-item label-width="130px" label="Firmware" prop='sourceUploadMethod'>
            <el-radio-group disabled size='small' v-model="form.sourceUploadMethod">
...
<script>
  import { editSolution, getModelList, getTags, solutionInfo } from '@/api/api.js'
  ...
  export default {
    ...
    methods: {
      submitForm(formName) {
        thisVue.$refs[formName].validate((valid) => {
          if(valid){
            ...
            editSolution(thisVue.form).then(json => {
      ...
      getSolutionInfo () {
        solutionInfo({id: thisVue.solution_id}).then(json => {
          let {id, image, is_public, link, model, model_id, platform, solution, tags, type, vendor, version} = json.data
          thisVue.form = {id, image, is_public: is_public.toString(), model_id, solution, version, tagsAttr: tags, url: link, sourceUploadMethod: '2'}
          thisVue.loading = false
        }).catch(err => {
          thisVue.loading = false
          return false
    ...
    mounted () {
      thisVue = this
      thisVue.solution_id = thisVue.$route.params.id
      thisVue.getModels()
      thisVue.getTagList()
      thisVue.getSolutionInfo()
    },
    watch: {
      form: {
